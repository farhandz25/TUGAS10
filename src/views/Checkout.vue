    ...
    <div class="container">
      ...
      <div class="checkout-content">
        ...
        <div class="checkout-step" v-if="currentStep === 3">
          ...
          <form @submit.prevent="goToStep(4)">
            <div class="form-group">
        ...
        <div class="checkout-step order-complete" v-if="currentStep === 4">
          ...
          <p>Terima kasih telah berbelanja di VapeStore. Kami telah mengirimkan detail pesanan ke email Anda.</p>
...
const orderNumber = ref('VS' + Math.floor(Math.random() * 1000000))
...
const filteredCities = computed(() => {
  ...
  return cities.value.filter(city => city.provinceId === province.id)
})
...
.payment-logo, .bank-logo {
  ...
  margin-bottom: 10px;
  object-fit: contain;
  filter: grayscale(100%) brightness(1.5); /* Membuat logo B/W */
  transition: filter var(--transition-speed);
}
.active .payment-logo, .active .bank-logo {
  filter: grayscale(0%) brightness(1); /* Warna asli saat aktif */
}
...
.btn-pay {
  background: var(--color-secondary); /* Warna hijau untuk tombol bayar */
