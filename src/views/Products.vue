<template>
  <div class="products-page">
    <div class="products-header">
      <div class="container">
        <h1>Produk Kami</h1>
        <p>Temukan koleksi lengkap produk vape terbaik</p>
      </div>
    </div>
...
const currentImage = ref(''); // Menggunakan URL gambar langsung, bukan index

const productImages = computed(() => {
  if (!selectedProduct.value) return [];
  // Jika nanti Anda punya galeri gambar di data produk (misal: product.gallery)
  // Anda bisa mengubahnya menjadi: return [selectedProduct.value.image, ...selectedProduct.value.gallery]
  return [selectedProduct.value.image]; // Untuk sekarang, hanya ada 1 gambar
});
...
const addToCartFromModal = () => {
  ...
  // Perlu membuat objek baru untuk menghindari reaktivitas aneh
  const productToAdd = { ...selectedProduct.value };
  
  cartStore.addToCart(productToAdd, quantity.value)
  toast.success(`${quantity.value}x "${productToAdd.name}" ditambahkan ke keranjang!`);
  closeQuickView()
}
...
.products-header {
  background: linear-gradient(rgba(18, 18, 18, 0.7), rgba(18, 18, 18, 0.9)), 
              url('@/assets/images/products-bg.jpg') no-repeat center center;
